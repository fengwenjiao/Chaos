...
cleanup() {
    contcfg --socket-path /tmp/contcfg-xrx.sock stop-server 
...
contcfg --socket-path /tmp/contcfg-xrx.sock start-server $MIN_MBPS $MAX_MBPS 10 &
contcfg_pid=$!
...
contcfg --socket-path /tmp/contcfg-xrx.sock ctrl add-all modelsynctime-worker 
