...
public:
    ...
    inline void start_server(int global_id){
        is_server_ = true;
        smq_thread_ =std::unique_ptr<std::thread>(new std::thread(std::bind(&Smq::server, this, global_id)));
    }
    ...
    inline void start_client(int global_id){
        is_server_ = false;
        smq_thread_ =std::unique_ptr<std::thread>(new std::thread(std::bind(&Smq::client, this, global_id)));
    }
    
    std::unordered_map<int,SmqMeta>  gather_info(int ksignal );

    inline void stop_smq(){
        while(!smq_ready_.load())std::this_thread::sleep_for(std::chrono::milliseconds(100));
