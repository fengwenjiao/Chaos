...
void BandwidthInfo::stop_bandwidth_test_server(){
    ...
    test_bandwidth("127.0.0.1", iperf_port_);
}
...
float BandwidthInfo::test_bandwidth(const std::string& ip,const int port){
    ...
    while(iperf_run_client(test)<0){
        ...
        LOG_WARNING_("Error when iperf test with: "<< ip<<":"<<port<<"   " << iperf_strerror(i_errno)<<", retrying...");
        sleep(2);
    }
    ...
    LOG_INFO_("test with: "<< ip<<" port:"<<port << ", bandwidth "<< band/(1000*1000) <<"Mbps");
    iperf_free_test(test);
    return band;
}
