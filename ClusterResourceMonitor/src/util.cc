...
string Util::open_file(const string& filePath) {
    ...
    if (!file.is_open()) {
        LOG_ERROR_("Failed to open " + filePath);
    }
...
string Util::find_value(const string&  text, const string& key, size_t pos){
    ...
    if(pos == string::npos){
        LOG_WARNING_("key not found: "+ key);
        return "";
    }
...
// #endif
    ...
    if(value_start_pos == wrap_pos){
        LOG_WARNING_("No corresponding value found: "+ key);
        return "";
    }
// #if DEBUG
...
string Util::remove_unit( const string& text){
    ...
    if (start_pos == string::npos) {
        LOG_WARNING_("No number found in the text: "+ text);
    }
if (end_pos == string::npos) {
        LOG_WARNING_("No unit found in the text: "+ text);
        end_pos = text.length();
    }
