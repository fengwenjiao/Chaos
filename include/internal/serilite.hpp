...
struct is_memcpy_constructible<
    T,
    std::enable_if_t<is_iterable_v<T> && std::is_trivially_copyable_v<typename T::value_type>>>
...
struct MemberCount {
  constexpr static size_t _impl(int*) {
...
void foreach_over_std_pair_tuple_impl(T& value, ExecFunc&& func, std::index_sequence<Is...>) {
...
void foreach_over_elements(T& value, ExecFunc&& func) {
  foreach_over_elements_impl(value, std::forward<ExecFunc>(func), constant_size_t<N>{});
}
...
template <auto N,
          ...
          std::enable_if_t<details::is_std_pairor_tuple_v<T>, int> = 0>
void foreach_over_elements(T& value, ExecFunc&& func) {
  foreach_over_std_pair_tuple_impl(
      value, std::forward<ExecFunc>(func), std::make_index_sequence<N>{});
}

// get a string_view of the serialized data of a string or vector-like type.
template <
    typename T,
    typename = std::void_t<decltype(std::declval<T>().data()), decltype(std::declval<T>().size())>>
...
struct SeriData {
   ...
   */
  const bytes_vector& as_vector() const {
    return data_;
  }
...
template <typename T>
std::enable_if_t<std::is_trivially_copyable_v<T>, SeriData> serialize(const T& value);

template <typename T>
std::enable_if_t<!std::is_trivially_copyable_v<T> && details::is_memcpy_constructible_v<T>,
                 SeriData>
...
std::enable_if_t<!std::is_trivially_copyable_v<T> &&
                     (std::is_aggregate_v<T> || details::is_std_pairor_tuple_v<T>),
                 SeriData>
serialize(const T& value);
...
serialize(const T& value) {
  ...
  for (const auto& element_data : temp) {
    std::memcpy(result.data_.data() + offset, element_data.data(), element_data.data_.size());
...
serialize(const T& value) {
  ...
  std::vector<SeriData> temp;
  size_t source_size = 0;
  details::foreach_over_elements<member_count>(value, [&temp, &source_size](const auto& element) {
    auto element_data = serialize(element);
...
template <typename T, typename U>
std::enable_if_t<std::is_trivially_copyable_v<T>, bool> deserialize(const U& serialized,
                                                                    T& value,
...
deserialize(const U& serialized, T& value, std::string_view* remain) {
  auto view = details::get_serialized_data(serialized);
  using value_type = typename T::value_type;
  if (view.size() < LengthSize)
    return false;
  LengthType element_count = 0;
  std::memcpy(&element_count, view.data(), LengthSize);
  size_t source_size = element_count * sizeof(value_type);
  if (view.size() < LengthSize + source_size)
    return false;
  const value_type* source = reinterpret_cast<const value_type*>(view.data() + LengthSize);
  value = T{source, source + element_count};
  if (remain)
    *remain = view.substr(LengthSize + source_size);
  return true;
}
...
deserialize(const U& serialized, T& value, std::string_view* remain) {
  ...
  for (size_t i = 0; i < element_count; ++i) {
    value_type element;
    if (!deserialize(ele_view, element, &ele_view))
      return false;
...
deserialize(const U& serialized, T& value, std::string_view* remain) {
  ...
  auto view = details::get_serialized_data(serialized);
  bool success = true;
  details::foreach_over_elements<member_count>(value, [&view, &success](auto& element) {
    if (!success)
      return;
